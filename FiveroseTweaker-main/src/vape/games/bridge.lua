return function(api)
	local run = function(func)
		...
		if not ok then
			...
			warn('[fiverosetweaker/bridge] '..tostring(err))
		end
	...
	local function child(parent, name, timeout)
		local value = parent:FindFirstChild(name) or parent:WaitForChild(name, timeout or 30)

		if not value then
			error('missing '..parent:GetFullName()..'.'..name, 0)
		end
	...
	child(lplr, 'PlayerGui', 30)

	local baseok, baseerr = run(function()
		local Knit = require(path(replicatedStorage, 'Modules', 'Knit', 'Client'))
		local timeout = os.clock() + 30

		while not debug.getupvalue(Knit.Start, 1) and os.clock() < timeout do
			task.wait()
		end

		if not debug.getupvalue(Knit.Start, 1) then
			error('Knit client did not start within 30 seconds', 0)
		end

		bd = setmetatable({
			BedwarsShop = require(path(replicatedStorage, 'Constants', 'BedWarsShop')),
			BedwarsUpgrades = require(path(replicatedStorage, 'Constants', 'BedWarsTeamUpgrades')),
			Blink = require(path(replicatedStorage, 'Blink', 'Client')),
			BreakTimes = require(path(replicatedStorage, 'Constants', 'Blocks')),
			BowClient = require(path(replicatedStorage, 'Client', 'Components', 'All', 'Tools', 'BowClient')),
			CombatConstants = require(path(replicatedStorage, 'Constants', 'Melee')),
			Communication = require(path(replicatedStorage, 'Client', 'Communication')),
			Knit = Knit,
			Entity = require(path(replicatedStorage, 'Modules', 'Entity')),
			ServerData = require(path(replicatedStorage, 'Modules', 'ServerData')),
		}, {
	...
	if not baseok then
		error('Bridge Duel bootstrap failed: '..tostring(baseerr), 0)
	end

	for _, name in ipairs({'Blink', 'Entity', 'ViewmodelController', 'MovementController', 'CombatService'}) do
		local ok, value = pcall(function()
			return bd[name]
		end)

		if not ok or value == nil then
			error('Bridge Duel bootstrap missing '..name..(ok and '' or ': '..tostring(value)), 0)
		end
	end

	if type(bd.Blink.item_action.attack_entity.fire) ~= 'function' then
		error('Bridge Duel attack remote is unavailable', 0)
	end
	...
	-- source: src/games/bridge duel/139566161526375 - game/Blatant/Criticals.lua
	run(function()
		local old
	...
	-- source: src/games/bridge duel/139566161526375 - game/Blatant/InvMove.lua
	run(function()
		local old
	...
	-- source: src/games/bridge duel/139566161526375 - game/Blatant/Killaura.lua
	run(function()
		local Killaura
		local Targets
		local CPS
	...
	-- source: src/games/bridge duel/139566161526375 - game/Blatant/NoFall.lua
	run(function()
		local old
	...
	-- source: src/games/bridge duel/139566161526375 - game/Blatant/NoSlowdown.lua
	run(function()
		local old
		local func
		...
		vape.Categories.Blatant:CreateModule({
			...
			Function = function(callback)
				if callback then
					func = debug.getproto(bd.MovementController.KnitStart, 7)

					if type(func) ~= 'function' then
						error('movement slowdown function is unavailable', 0)
					end

					old = debug.getconstants(func)

					if type(old) ~= 'table' then
						error('movement slowdown constants are unavailable', 0)
					end
	...
	-- source: src/games/bridge duel/139566161526375 - game/Blatant/ProjectileAimbot.lua
	run(function()
		local TargetPart
		local FOV
		local old
		...
		local ProjectileAimbot = vape.Categories.Blatant:CreateModule({
			...
			Function = function(callback)
				if callback then
					hooked = debug.getupvalue(bd.BowClient.Start, 11)

					if type(hooked) ~= 'function' then
						error('bow targeting function is unavailable', 0)
					end

					old = hookfunction(hooked, function(...)
						return aimFunction(...)
					end)
				elseif old and hooked then
	...
	-- source: src/games/bridge duel/139566161526375 - game/Combat/AutoClicker.lua
	run(function()
		local AutoClicker
		local CPS
	...
	-- source: src/games/bridge duel/139566161526375 - game/Combat/Reach.lua
	run(function()
		--[[local old
	...
	-- source: src/games/bridge duel/139566161526375 - game/Combat/Velocity.lua
	run(function()
		local Velocity
		local Horizontal
		local Vertical
		...
		Velocity = vape.Categories.Combat:CreateModule({
			...
			Function = function(callback)
				if callback then
					...
					local ok, connections = pcall(getconnections, bd.CombatService.KnockBackApplied._re.OnClientEvent)
					connection = ok and type(connections) == 'table' and connections[1]

					if not connection or type(connection.Function) ~= 'function' then
						error('knockback connection is unavailable', 0)
					end
	...
	-- source: src/games/bridge duel/139566161526375 - game/Inventory/AutoBuy.lua
	run(function()
		local AutoBuy
		local Sword
		local Armor
	...
	-- source: src/games/bridge duel/139566161526375 - game/Minigames/Breaker.lua
	run(function()
		local Breaker
		local Range
		local Value
	...
	-- source: src/games/bridge duel/139566161526375 - game/Utility/AutoPlay.lua
	run(function()
		local AutoPlay
		local Delay
	...
	-- source: src/games/bridge duel/139566161526375 - game/Utility/Scaffold.lua
	run(function()
		local Scaffold
		local Expand
		local Tower
