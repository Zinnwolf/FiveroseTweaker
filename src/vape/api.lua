return function(api, entry)
	...
	local function restoretabs()
		for _, name in ipairs(names) do
			...
			if tab and type(rawget(tab, 'Destroy')) == 'function' then
		...
		for index = #paused, 1, -1 do
			...
			local toggle = api.toggles[data.id]
			...
			local drop = makeoption(module, 'Dropdown', {
				...
				Function = function(value)
					...
					if font then
						...
						safe(module.Name..'/'..name, settings.Function, Font.fromEnum(font))
					end
				end
			})
			local obj = drop
	...
	local function module(category, settings, isoverlay)
		...
		local function changed()
			...
			if not obj._dead then
				safe(name, settings.Function, value)
			end
		end
	...
	api.vape = vape
	api.vapelibs = {
		entity = fresh('src/vape/libraries/entity.lua'),
		prediction = fresh('src/vape/libraries/prediction.lua'),
		hash = fresh('src/vape/libraries/hash.lua'),
		vm = fresh('src/vape/libraries/vm.lua'),
		drawing = fresh('src/vape/libraries/drawing.lua')
	}
