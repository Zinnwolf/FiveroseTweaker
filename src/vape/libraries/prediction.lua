...
local function solveLinearIntercept(disp, velocity, speed)
	if not isFinite(speed) or speed <= eps then
		return
	end

	local a = velocity:Dot(velocity) - speed * speed
	local b = 2 * disp:Dot(velocity)
	...
	if math.abs(a) <= eps then
		...
		if discriminant >= 0 and isFinite(discriminant) then
	...
	for _, value in roots do
		if isFinite(value) and value > eps and (not earliest or value < earliest) then
			earliest = value
		end
	end
...
function module.SolveTrajectory(origin, projectileSpeed, gravity, targetPos, targetVelocity, playerGravity, playerHeight, playerJump, params)
	...
	if math.abs(gravity) <= eps then
		local time = solveLinearIntercept(disp, targetVelocity, projectileSpeed)

		if time then
			return origin + (disp + targetVelocity * time) / time
		end
