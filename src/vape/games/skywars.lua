return function(api)
	local run = function(func)
		...
		if not ok then
			...
			warn('[fiverosetweaker/skywars] '..tostring(err))
		end
	...
	required(function()
		...
		if not debug.getupvalue(Flamework.ignite, 1) then
			...
			repeat task.wait() until debug.getupvalue(Flamework.ignite, 1) or vape.Loaded == nil or os.clock() >= timeout
			if vape.Loaded == nil then return end
			if not debug.getupvalue(Flamework.ignite, 1) then
				error('SkyWars Flamework did not ignite within 30 seconds', 0)
			end
		end
		...
		for _, name in ipairs({'CameraUtil', 'FireOrigin', 'Gravity', 'ItemMeta', 'Remotes', 'Store', 'Shop'}) do
			if skywars[name] == nil then
				error('SkyWars bootstrap missing '..name, 0)
			end
		end

		for _, name in ipairs({'updateActiveItem', 'strikeDesktop'}) do
			if remotes[name] == nil then
				error('SkyWars remote discovery missing '..name, 0)
			end
		end
	...
	-- source: src/games/skywars voxel/8768229691 - skywars game/Blatant/AntiFall.lua
	run(function()
		local AntiFall
		local Mode
		local Material
	...
	-- source: src/games/skywars voxel/8768229691 - skywars game/Blatant/InvMove.lua
	run(function()
		local InvMove
		local old
	...
	-- source: src/games/skywars voxel/8768229691 - skywars game/Blatant/Killaura.lua
	run(function()
		local Killaura
		local Targets
		local AttackRange
	...
	-- source: src/games/skywars voxel/8768229691 - skywars game/Blatant/NoFall.lua
	run(function()
		local NoFall
	...
	-- source: src/games/skywars voxel/8768229691 - skywars game/Blatant/NoSlowdown.lua
	run(function()
		local old, oldcheck
	...
	-- source: src/games/skywars voxel/8768229691 - skywars game/Blatant/ProjectileAimbot.lua
	run(function()
		local TargetPart
		local FOV
		local old, oldMobile
	...
	-- source: src/games/skywars voxel/8768229691 - skywars game/Blatant/ProjectileAura.lua
	run(function()
	...
	-- source: src/games/skywars voxel/8768229691 - skywars game/Combat/AutoClicker.lua
	run(function()
		local AutoClicker
		local CPS
		local Blocks
	...
	-- source: src/games/skywars voxel/8768229691 - skywars game/Combat/Sprint.lua
	run(function()
		local Sprint
		local old
	...
	-- source: src/games/skywars voxel/8768229691 - skywars game/Combat/Velocity.lua
	run(function()
		local Velocity
		local Horizontal
		local Vertical
	...
	-- source: src/games/skywars voxel/8768229691 - skywars game/Inventory/AutoBuy.lua
	run(function()
		local AutoBuy
		local Sword
		local Armor
	...
	-- source: src/games/skywars voxel/8768229691 - skywars game/Inventory/AutoConsume.lua
	run(function()
		local AutoConsume
	...
	-- source: src/games/skywars voxel/8768229691 - skywars game/Legit/Viewmodel.lua
	run(function()
		local Viewmodel
		local oldtool
	...
	-- source: src/games/skywars voxel/8768229691 - skywars game/Minigames/Breaker.lua
	run(function()
		local Breaker
		local Range
		local BreakerPart
	...
	-- source: src/games/skywars voxel/8768229691 - skywars game/Utility/Scaffold.lua
	run(function()
		local Scaffold
		local Expand
		local Tower
	...
	-- source: src/games/skywars voxel/8768229691 - skywars game/World/ChestSteal.lua
	run(function()
		local ChestSteal
		local Range
		local Open
