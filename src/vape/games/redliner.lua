return function(api)
	...
	local run = function(func)
		...
		if not ok then
			...
			warn('[fiverosetweaker/redliner] '..tostring(err))
		end
	...
	local getcustomasset = getcustomasset or vape.Libraries.getcustomasset
	local redline = {Teams = {}}
	...
	local function warningRoutine()
		notif('Redliner', 'Game update hash differs from the imported snapshot', 8, 'warning')
	end

	-- Redliner's client classes live in a Roblox actor. This bundled loader has no
	-- safe way to reinject itself into that actor, so only continue when the
	-- executor already exposes the actor-owned scripts and closures to this VM.
	local extractionApis = {
		debug_constants = debug and debug.getconstants,
		debug_info = debug and debug.info,
		debug_protos = debug and debug.getprotos,
		debug_upvalue = debug and debug.getupvalue,
	...
	local missingApis = {}
	for name, value in extractionApis do
	...
	if #missingApis > 0 then
		table.sort(missingApis)
		local reason = 'missing executor APIs: '..table.concat(missingApis, ', ')
		warn('[fiverosetweaker/redliner] compatibility gate: '..reason)
		notif('Redliner', 'Compatibility gate blocked Redliner ('..reason..').', 10, 'alert')
		return
	end
	...
	for _ = 1, 3 do
		...
		if ok and type(modules) == 'table' then
			for _, module in modules do
				if typeof(module) == 'Instance' and module:GetFullName() == 'Start.Client.ClientRoot' then
					rootVisible = true
					break
				end
			end
	...
	if not rootVisible then
		local reason = 'Start.Client.ClientRoot is not visible from the caller VM; an actor/thread bridge is required'
		warn('[fiverosetweaker/redliner] compatibility gate: '..reason)
		notif('Redliner', 'Compatibility gate blocked Redliner: actor-owned closures are unavailable.', 10, 'alert')
		return
	end

	local entityok, entityerr = run(function()
	...
	if not entityok then
		error('Redliner entity bootstrap failed: '..tostring(entityerr), 0)
	end
	entitylib.start()
	
	local bootstrapok, bootstraperr = run(function()
		local root
	...
	if not bootstrapok then
		error('Redliner client bootstrap failed: '..tostring(bootstraperr), 0)
	end
	
	for _, name in {'Packet', 'AttackPacket', 'MoveController', 'LaunchpadFunction', 'DashRecoverVariable', 'DashVariable', 'ShootFunction', 'ActionController', 'ActionFunction'} do
		if redline[name] == nil then
			error('Redliner client bootstrap missing '..name, 0)
		end
	end
	
	if redline[redline.MoveController] == nil then
		error('Redliner movement controller is unavailable', 0)
	end
	...
	-- source: src/games/redliner/115875349872417 - game/Blatant/AntiParry.lua
	run(function()
		local AntiParry
		local anims = {
	...
	-- source: src/games/redliner/115875349872417 - game/Blatant/AutoParry.lua
	run(function()
		local AutoParry
	...
	-- source: src/games/redliner/115875349872417 - game/Blatant/Fly.lua
	run(function()
		local Fly
		local LongJump
		run(function()
	...
	-- source: src/games/redliner/115875349872417 - game/Blatant/HighJump.lua
	run(function()
		local HighJump
		local Value
	...
	-- source: src/games/redliner/115875349872417 - game/Blatant/InfiniteDash.lua
	run(function()
		local InfiniteDash
	...
	-- source: src/games/redliner/115875349872417 - game/Blatant/Killaura.lua
	run(function()
		local Killaura
		local Targets
		local AttackRange
	...
	-- source: src/games/redliner/115875349872417 - game/Combat/Reach.lua
	run(function()
		local Reach
	...
	-- source: src/games/redliner/115875349872417 - game/Combat/SilentAim.lua
	run(function()
		local SilentAim
		local Target
		local Range
	...
	-- source: src/games/redliner/115875349872417 - game/Legit/HitSound.lua
	run(function()
		local HitSound
		local Value
		local Volume
	...
	-- source: src/games/redliner/115875349872417 - game/Legit/KillSound.lua
	run(function()
		local KillSound
		local Value
		local Volume
	...
	-- source: src/games/redliner/115875349872417 - game/Utility/AutoQueue.lua
	run(function()
		local AutoQueue
	...
	-- source: src/games/redliner/115875349872417 - game/Utility/AutoToxic.lua
	run(function()
		local AutoToxic
		local GG
