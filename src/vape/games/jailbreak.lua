return function(api)
	local run = function(func)
		...
		if not ok then
			...
			warn('[fiverosetweaker/jailbreak] '..tostring(err))
		end
	...
	required(function()
		...
		if not jb.VehicleController.toggleLocalLocked or not jb.VehicleController.NitroShopVisible then
			...
			repeat task.wait() until (jb.VehicleController.toggleLocalLocked and jb.VehicleController.NitroShopVisible) or vape.Loaded == nil or os.clock() >= timeout
			if vape.Loaded == nil then return end
			if not (jb.VehicleController.toggleLocalLocked and jb.VehicleController.NitroShopVisible) then
				error('Jailbreak vehicle controller did not become ready within 30 seconds', 0)
			end
		end
	...
	-- source: src/games/jailbreak/606849621 - main/Blatant/AutoArrest.lua
	run(function()
		local AutoArrest
	...
	-- source: src/games/jailbreak/606849621 - main/Blatant/AutoPop.lua
	run(function()
		local AutoPop
		local Range
		local TeamCheck
	...
	-- source: src/games/jailbreak/606849621 - main/Blatant/AutoPunch.lua
	run(function()
		local AutoPunch
	...
	-- source: src/games/jailbreak/606849621 - main/Blatant/AutoTaze.lua
	run(function()
		local AutoTaze
		local HandCheck
	...
	-- source: src/games/jailbreak/606849621 - main/Blatant/LazerGodmode.lua
	run(function()
		LazerGodmode = vape.Categories.Blatant:CreateModule({Name = 'LazerGodmode'})
	end)

	-- source: src/games/jailbreak/606849621 - main/Blatant/NoFall.lua
	run(function()
	...
	-- source: src/games/jailbreak/606849621 - main/Combat/ForceHeadshot.lua
	run(function()
		local ForceHeadshot
		local old
		...
		ForceHeadshot = vape.Categories.Combat:CreateModule({
			...
			Function = function(callback)
				if callback then
					old = hookfunction(jb.GunController.BulletEmitterOnLocalHitPlayer, function(...)
				...
				elseif old then
					if type(restorefunction) == 'function' then
						...
						hookfunction(jb.GunController.BulletEmitterOnLocalHitPlayer, old)
					end
	...
	-- source: src/games/jailbreak/606849621 - main/Combat/SilentAim.lua
	run(function()
		local SilentAim
		local Target
		local Mode
	...
	-- source: src/games/jailbreak/606849621 - main/Utility/InfiniteNitro.lua
	run(function()
	...
	-- source: src/games/jailbreak/606849621 - main/Utility/InstantAction.lua
	run(function()
