return function(api)
	local run = function(func)
		...
		if not ok then
			...
			warn('[fiverosetweaker/flee] '..tostring(err))
		end
	...
	required(function()
		...
		if not lstats then
			...
			repeat
				lstats = lplr:FindFirstChild('TempPlayerStatsModule')
				task.wait()
			until lstats or vape.Loaded == nil or os.clock() >= timeout
			...
			if not lstats then
				error('Flee player stats did not become ready within 30 seconds', 0)
			end
		end
	...
	-- source: src/games/893973440 - flee the facility/Blatant/NoSlowdown.lua
	run(function()
		local NoSlowdown
		local old
	...
	-- source: src/games/893973440 - flee the facility/Blatant/PhaseHammer.lua
	run(function()
		local PhaseHammer
		local old
	...
	-- source: src/games/893973440 - flee the facility/Blatant/RestrainBeast.lua
	run(function()
		local RestrainBeast
	...
	-- source: src/games/893973440 - flee the facility/Blatant/SlowBeast.lua
	run(function()
		local SlowBeast
	...
	-- source: src/games/893973440 - flee the facility/Blatant/SpamBeast.lua
	run(function()
		local SpamBeast
	...
	-- source: src/games/893973440 - flee the facility/Render/ComputerESP.lua
	run(function()
		local ComputerESP
		local FillColor
		local OutlineColor
	...
	-- source: src/games/893973440 - flee the facility/Utility/AutoComputer.lua
	run(function()
		local AutoComputer
