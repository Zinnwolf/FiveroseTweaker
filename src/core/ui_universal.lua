return function(api, root)
	...
	local function wrapkey(obj, id, owned)
		...
		local function set(value)
			if blocked(value) then
				local items = rawget(obj, 'Items')
				local button = type(items) == 'table' and rawget(items, 'Keybind')
				local name = keyname(rawget(obj, 'Key'))

				if alive(button) then
					button.Text = '['..(name == 'None' and '' or name)..']'
				end
	...
	function state:wrapkey(obj, id)
		return wrapkey(obj, id, false)
	end
